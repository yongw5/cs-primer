...
## sentinelFlushConfig()
sentinelFlushConfig() 函数用于将 Sentinel 当前的配置写入 server.configfile 中。
```
// sentinel.c
...
```
rewriteConfig() 将重写配置文件 server.configfile。 如果配置文件已经存在，将尽最大努力保留注释和整体结构，如果 server.configfile 中已经存在配置参数，这些值将保留。
```
// config.c
